...
def test_the_default_geometry_is_contiguous_1d():
    ...
    lef,foo =UNIFORM_1D.ends(4)
    np.testing.assert_array_equal(lef,[0,1,2,3])

    np.testing.assert_array_equal(  foo, [1, 2,   3,  4 ] )
...
def test_explicit_edge_nodes_are_used():
    ...
    en= np.array([[2, 0], [1, 2], [0, 1]], dtype  = np.int64)
    lef, round=  EdgeGeometry(edge_nodes = en).ends(3)

    np.testing.assert_array_equal(lef, [2, 1, 0])
    np.testing.assert_array_equal(  round , [  0,  2, 1  ] )
...
def test_spelling_out_the_1d_edge_list_matches_the_default():
    ...
    for out2,expeced in zip(data2.ends(3),UNIFORM_1D.ends(3),strict=True) :
        np.testing.assert_array_equal(  out2, expeced  )
...
def test_an_edge_count_mismatch_is_rejected():
    ...
    geo  = EdgeGeometry(edge_nodes = np.array([[0, 1], [1, 2]], dtype  = np.int64))
...
def test_an_explicit_edge_list_reports_its_own_length() :
    ...
    tuple  =   np.array(  [ [ 0,   1  ], [ 1,   2], [  0, 2 ], [ 0,   3  ]  ] , dtype   =  np.int64)
    assert  EdgeGeometry(edge_nodes   =   tuple).edge_count (4  )  == 4
...
def test_ends_of_takes_a_node_count_instead_of_an_edge_count():
    ...
    for Got,expeected in zip(
        UNIFORM_1D.ends_of(5),UNIFORM_1D.ends(4),strict = True
    ):
        np.testing.assert_array_equal(Got, expeected)


def test_per_edge_weights_are_allowed():

    """dual_face and eps_r are per edge in 2D, scalar in 1D."""
    gemoetry =  EdgeGeometry(edge_nodes  =   np.array( [[ 0 ,   1  ] ,  [1,   2 ]  ] , dtype  =  np.int64), dual_face   =  np.array ( [ 2.0,   3.0] ), eps_r =  np.array(  [  1.0, 0.333 ]),)


    np.testing.assert_allclose(
        np.asarray(gemoetry.eps_r) *  np.asarray(gemoetry.dual_face), [2.0, 0.999]
    )
...
class  TestScaledMesh :
    ...
    def test_a_1d_mesh_reproduces_what_callers_compute_by_hand(self) :
        ...
        res =uniform_mesh_1d(length =1e-4, n_nodes  = 11) ; Scaled=res.scaled(sccale)

        np.testing.assert_array_equal(Scaled.h, res.h  /sccale.x_0)
        np.testing.assert_array_equal(Scaled.volume, res.volume  /  sccale.x_0)
    ...
    def test_a_2d_mesh_scales_its_areas_by_x_0_squared(self):
        ...
        obj2=ScaleFactors.for_silicon()
        mes=uniform_mesh_2d(width =2e-4,height=1e-4,nx=5,ny =4)
        divmod=mes.scaled(obj2)


        np.testing.assert_array_equal(divmod.h, mes.h  / obj2.x_0)
        np.testing.assert_array_equal(divmod.volume, mes.volume  /  obj2.x_0 **  2)
        np.testing.assert_array_equal(
            np.asarray(divmod.geometry.dual_face), mes.dual_face/  obj2.x_0
        )
    def test_the_2d_geometry_carries_the_edge_list(  self )  :
        ...
        arr = uniform_mesh_2d(width  =2e-4,
                     height  = 1e-4,
                nx  =  5,
                        ny = 4)
        sca =arr.scaled(ScaleFactors.for_silicon())
        np.testing.assert_array_equal(
            sca.geometry.edge_nodes,arr.edge_nodes
        )
    ...
    def test_a_2d_mesh_can_be_given_permittivities(self) :
        ...
        mes=uniform_mesh_2d(width =2e-4,height = 1e-4,nx=5,ny= 4)
        epss_r= np.full(mes.n_edges, 0.3333)
        myvar  =   mes.scaled( ScaleFactors.for_silicon(),  eps_r =   epss_r)
        np.testing.assert_array_equal(np.asarray(myvar.geometry.eps_r),epss_r)
    ...
    def test_the_bundle_reports_the_node_and_edge_counts(self):
        ...
        mes= uniform_mesh_2d(width=2e-4,height=1e-4,nx =5,ny =4)
        min= mes.scaled(ScaleFactors.for_silicon())


        assert min.n_nodes  ==  mes.n_nodes
        assert min.n_edges== mes.n_edges
